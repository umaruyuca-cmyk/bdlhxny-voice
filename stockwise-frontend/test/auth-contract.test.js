...
test("工作站提供登录和一键账号申请，并为 API 自动携带 JWT", async () => {
  ...
  assert.match(html, /id="authApplyButton"/);
  assert.match(html, /一键申请账号/);
  assert.match(script, /\/api\/v1\/auth\/apply/);
...
test("浏览器会话和标的缓存按登录用户隔离", async () => {
  ...
  assert.match(script, /userStorageKey\("stockwise\.currentInstrument"\)/);
...
test("新版统一聊天页复用账号体系并按用户隔离本地会话", async () => {
  ...
  assert.match(html, /id="authApply"/);
  assert.match(script, /\/api\/v1\/auth\/apply/);
  assert.match(script, /\/api\/v1\/auth\/login/);
  ...
  assert.match(server, /STOCKWISE_ANALYSIS_URL/);
