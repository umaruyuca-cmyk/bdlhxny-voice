...
不保留增量迁移、旧表兼容或历史数据回填脚本。所有脚本均需由开发人员或
...
3. 使用 `bdlh_runtime_data` 角色执行 `postgresql/schema/platform_contract.sql`、`runtime_core.sql`、`task_messaging.sql`、`notifications.sql`、`run_projection.sql` 和 `registry.sql`。
