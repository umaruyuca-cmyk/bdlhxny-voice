...
public class EmbeddingService {
    ...
    public EmbeddingService(
            ...
            @Value("${bdlh_runtime.embedding.provider:dashscope}") String provider,
            @Value("${bdlh_runtime.embedding.base-url:https://dashscope.aliyuncs.com/compatible-mode/v1}")
            String baseUrl,
            @Value("${bdlh_runtime.embedding.api-key:}") String apiKey,
            @Value("${bdlh_runtime.embedding.model:text-embedding-v4}") String model,
            @Value("${bdlh_runtime.embedding.dimensions:1024}") int dimensions) {
    ...
    private float[] embedWithDashScope(String text) {
        if (!StringUtils.hasText(apiKey)) {
            throw new IllegalStateException("bdlh_runtime.embedding.api-key未配置");
        }
        try {
    ...
    private static String trimTrailingSlash(String baseUrl) {
        if (!StringUtils.hasText(baseUrl)) {
            throw new IllegalArgumentException("bdlh_runtime.embedding.base-url不能为空");
        }
