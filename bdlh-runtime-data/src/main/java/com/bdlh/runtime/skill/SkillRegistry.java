...
public class SkillRegistry {
    ...
    private SkillDefinition portfolioAnalysis() {
        return new SkillDefinition(
                ...
                "持仓组合分析",
                """
                你是 BDLH Agent Runtime 的持仓组合分析师，负责用户整体持仓评估。
                工作规则：
    ...
    private SkillDefinition quantAnalysis() {
        ...
        return variant(
                ...
                "ETF 多标的量化轮动",
                """
                你是 BDLH Agent Runtime 的 ETF 量化分析师，只解释已经校验的 quant JSON。
                工作规则：
    ...
    private SkillDefinition sectorAnalysis() {
        ...
        return variant(
                ...
                "行业与概念板块分析",
                """
                你是 BDLH Agent Runtime 的板块分析师，只解释已经校验的 sector JSON。
                工作规则：
    ...
    private SkillDefinition sectorAttention() {
        ...
        return variant(
                ...
                "板块行情与外围关注代理",
                """
                你是 BDLH Agent Runtime 的板块外围关注分析师，只解释已校验的 sector JSON、固定 SearchResult 和 attentionSnapshot。
                工作规则：
    ...
    private SkillDefinition causalAnalysis() {
        ...
        return variant(
                ...
                "外部事件市场影响分析",
                """
                你是 BDLH Agent Runtime 的市场因果分析师，只能依据已校验的行情或板块数据和外部证据解释影响。
                工作规则：
    ...
    private SkillDefinition stockDeepAnalysis() {
        return new SkillDefinition(
                ...
                "单标的深度分析",
                """
                你是 BDLH Agent Runtime 的单标的研究分析师。StockSkill 负责确定性数据计算，你负责将证据解释为可执行、可复核的研究判断。
                工作规则：
