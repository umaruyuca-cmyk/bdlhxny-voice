...
> **约束**：本文档必须与 `docs/00-PRD-v3.md` 保持一致，后者为需求唯一真源，冲突时以 PRD v3 为准。
...
> - **游客分析次数门禁（2026-07-31 实施）**：游客不限制 Token，只限制分析请求次数，默认最多 10 次。计数闸门位于 RouteDecision 之后、任何 Skill/WebSearch/付费模型之前；普通问答和行情事实不扣次数。
...
18. 游客不设置 Token 配额；只有深度分析 Route 消耗提问次数，默认上限为 10。第 11 次必须在真实工具和付费模型前拒绝。
...
| 文件 | 说明 |
|------|------|
| `docs/00-PRD-v3.md` | 需求唯一真源：Route-Intent-Skill、付费门禁、知识生命周期、API、护栏、DB 设计 |
