...
> **审查规范：[00-StockWise生产审查规范.md](./00-StockWise生产审查规范.md)**
...
| Python 生产配置 | 静态核对 `STOCKWISE_ENV`、Postgres Checkpointer、JWT、外部服务地址 | 云端 Compose 已显式注入 |
...
- Python API 已支持 JWT 校验，生产 Compose 设置 `STOCKWISE_AUTH_REQUIRED=true`；
- 会话、运行与历史相关测试覆盖认证隔离；
