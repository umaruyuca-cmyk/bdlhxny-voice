# web-search-wrapper 云端部署

## 1. 构建

```bash
docker build -t stockwise-web-search-wrapper:0.1.0 .
```
...
docker run -d \
  ...
  -e 'WEB_SEARCH_AGENTS_JSON={"stockwise":"替换为至少32位的独立Token"}' \
  stockwise-web-search-wrapper:0.1.0
```
