...
| API 代理 | 认证/用户 API → Java `127.0.0.1:8081`（`BDLH_RUNTIME_BACKEND_URL`）；聊天/会话 → Python `127.0.0.1:8000`（`BDLH_RUNTIME_ANALYSIS_URL`）。生产 Python 使用 `127.0.0.1:8090` |
...
2. `cd bdlh-runtime-console && node dev-server.js`（默认 8082）。
