    ...
    def build(
        ...
        *,
        ...
        if positions_observation is not None:
            positions = [
                PortfolioPosition.model_validate(item)
                for item in positions_observation.data.get("positions", [])
            ]
            if any(
                item.market_value is None or item.weight_pct is None
                for item in positions
            ):
                limitations.append(
                    "Current market_value/weight_pct missing; concentration unavailable"
                )
        ...
        if account_observation is not None:
            ...
            if account.total_assets is None:
                limitations.append("Account total_assets missing")
