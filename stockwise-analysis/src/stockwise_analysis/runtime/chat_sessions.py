...
class PostgresChatSessionStore:
    ...
    def _setup(self) -> None:
        with self._connect() as connection:
            connection.execute("""
                CREATE TABLE IF NOT EXISTS stockwise_chat_session (
            ...
            """)
            connection.execute("""
                CREATE TABLE IF NOT EXISTS stockwise_chat_message (
                    ...
                    FOREIGN KEY (user_id, session_id)
                        REFERENCES stockwise_chat_session(user_id, session_id)
                        ON DELETE CASCADE
                )
            """)
            connection.execute("""
                CREATE INDEX IF NOT EXISTS idx_stockwise_chat_message_session
                ON stockwise_chat_message(user_id, session_id, id)
            """)

    def _session_from_row(self, connection, row) -> ChatSession:
        messages = connection.execute(
            """
            SELECT role, content FROM stockwise_chat_message
    ...
    def ensure(self, requested_id: str | None, user_id: str | None) -> ChatSession:
        ...
        with self._connect() as connection:
            connection.execute(
                """
                INSERT INTO stockwise_chat_session(user_id, session_id)
                VALUES (%s, %s)
                """,
                (owner, session_id),
            )
    ...
    def add_message(self, session_id: str, user_id: str | None, role: str, content: str) -> None:
        ...
        with self._connect() as connection:
            row = connection.execute(
                "SELECT title FROM stockwise_chat_session WHERE user_id = %s AND session_id = %s",
                (owner, session_id),
            ).fetchone()
            if row is None:
                raise KeyError(f"chat session not found: {session_id}")
            connection.execute(
                """
                INSERT INTO stockwise_chat_message(user_id, session_id, role, content)
                VALUES (%s, %s, %s, %s)
                """,
            ...
            connection.execute(
                """
                UPDATE stockwise_chat_session SET title = %s, updated_at = CURRENT_TIMESTAMP
                WHERE user_id = %s AND session_id = %s
                """,
    ...
    def prepare_regeneration(self, session_id: str, user_id: str | None) -> None:
        ...
        with self._connect() as connection:
            existing = connection.execute(
                "SELECT 1 FROM stockwise_chat_session WHERE user_id = %s AND session_id = %s",
                (owner, session_id),
            ).fetchone()
            if existing is None:
                raise KeyError(f"chat session not found: {session_id}")
            connection.execute(
                """
                DELETE FROM stockwise_chat_message
                WHERE id = (
                    SELECT id FROM stockwise_chat_message
                    WHERE user_id = %s AND session_id = %s
            ...
            connection.execute(
                """
                UPDATE stockwise_chat_session SET updated_at = CURRENT_TIMESTAMP
                WHERE user_id = %s AND session_id = %s
                """,
                (owner, session_id),
            )
    ...
    def delete(self, session_id: str, user_id: str | None) -> bool:
        ...
        with self._connect() as connection:
            cursor = connection.execute(
                "DELETE FROM stockwise_chat_session WHERE user_id = %s AND session_id = %s",
                (owner, session_id),
            )
