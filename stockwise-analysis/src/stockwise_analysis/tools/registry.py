...
class ToolRegistry:
    ...
    def register(self, name: str, description: str, handler: ToolHandler, *, read_only: bool = True, timeout_seconds: int = 20) -> None:
        """注册只读工具；第一阶段拒绝任何写操作。"""
        if name in self._tools:
            raise ValueError(f"Tool already registered: {name}")
        if not read_only:
            raise ValueError("The first workflow version only permits read-only tools")
        self._tools[name] = ToolSpec(name=name, description=description, handler=handler, read_only=read_only, timeout_seconds=timeout_seconds)
