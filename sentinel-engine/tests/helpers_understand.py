...
class RuleBasedUnderstandModel:
    ...
    async def understand(
        ...
        *,
        ...
        out = rule_based_understand(message)
        action = _rule_action(out, enabled_skills)
        return out.model_copy(update={"action": action})


def _rule_action(out: UnderstandOutput, enabled_skills: frozenset[str] | None) -> ActionSpec | None:
    """测试替身：会话有可用工具且立案带标的/账户/市场主题时输出 action。"""
    allowed = enabled_skill_ids(enabled_skills)
    if not allowed:
        return None

    # 按消息特征选择 tool（模拟 LLM 决策）
    message = ""
    for goal in out.goals:
        message = goal.objective + " "
    message = message.strip()

    if "suitability-evaluation" in allowed[0] or _SUITABILITY_PATTERN.search(message):
        symbol = out.entities.instruments[0] if out.entities.instruments else ""
        return ActionSpec(tool="suitability-evaluation", parameters={"symbol": symbol})
    topics = out.goals[0].requested_topics if out.goals else []
    if "portfolio-health" in allowed[0] or any(t in {"news", "money_flow", "industry"} for t in topics):
        return ActionSpec(tool="portfolio-health", parameters={})
    if "stock-research" in allowed[0] and out.entities.instruments:
        return ActionSpec(tool="stock-research", parameters={"symbol": out.entities.instruments[0]})

    # fallback：有标的走 research，否则走 portfolio
    if out.entities.instruments:
        return ActionSpec(tool="stock-research", parameters={"symbol": out.entities.instruments[0]})
    return ActionSpec(tool="portfolio-health", parameters={})
