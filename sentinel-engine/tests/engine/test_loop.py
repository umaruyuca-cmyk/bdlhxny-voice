...
Observation 回填、历史裁剪、L3 记忆注入、无 LLM degraded。
"""
...
def test_load_scoped_portfolio_requires_auth(registry_snapshot):
    ...
    assert "memory.recall" in host
    assert "memory.recall" not in guest
...
@pytest.mark.asyncio
async def test_memory_injected_into_message_sequence(registry_snapshot):
    ...
    await loop.run(
        AgentTurn(
            user_id="u1",
            message="对我的计划有影响吗",
            memory_records=[MemoryRecord(content="两年内换房")],
        )
    )
