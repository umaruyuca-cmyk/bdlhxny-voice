...
def test_cognitive_and_guardrails_have_no_domain_literal():
    """新增领域时 cognitive/ 与 guardrails/ 不得出现 finance / weather 字面量。"""
    import re

    banned = re.compile(r"\b(finance|weather)\b", re.IGNORECASE)
    hits: list[str] = []
    for folder in ("cognitive", "guardrails"):
        for path in sorted((SRC_ROOT / folder).rglob("*.py")):
            for lineno, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
                if banned.search(line):
                    hits.append(f"{path.relative_to(SRC_ROOT)}:{lineno}:{line.strip()}")
    assert not hits, "内核渗漏领域字面量：\n" + "\n".join(hits)
