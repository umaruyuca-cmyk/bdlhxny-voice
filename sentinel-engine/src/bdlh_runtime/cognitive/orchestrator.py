...
from bdlh_runtime.guardrails.interfaces import PlanGuardrail
...
from .checkpoint import CognitiveCheckpoint, ResumeCursor, build_checkpoint
...
class CognitiveOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        plan_guardrail: PlanGuardrail[CognitiveAction, CognitiveAction] | None = None,
    ...
    async def run(
        ...
        *,
        ...
        understood = await self._load_understand(event)
        allowed_names = sorted(self._authorized_capabilities)
        goals = backfill_criteria(list(understood.goals), allowed_names) if understood is not None else []
        state = CognitiveState(
            event=event,
            goals=goals,
            needs_external=bool(understood.needs_external) if understood is not None else False,
            error_codes=list(understood.reason_codes) if understood is not None else [],
        )
    ...
    async def _load_understand(self, event: InputEvent) -> UnderstandOutput | None:
        """唤醒事件不经 Understand；其余轮次都立案，并把本轮工具列表交给 LLM。"""
        if event.event_type == InputEventType.SCHEDULED_WAKEUP:
            return None
        return await self._understand.understand(event.message, enabled_skills=event.enabled_skills)
    ...
    async def _continue_from_outcome(
        ...
        *,
        ...
        execution_observer.on_domain_outcome(outcome)
        ...
        state.goals, coverage_assumed = _refresh_goal_coverage(
            state.goals,
            outcome=outcome,
            allowed=sorted(self._authorized_capabilities),
        )
        if coverage_assumed and "GOAL_COVERAGE_ASSUMED" not in state.error_codes:
            state.error_codes.append("GOAL_COVERAGE_ASSUMED")
    ...
    def _finalize(
        ...
        *,
        ...
        audit_codes = list(dict.fromkeys([audit_code, *state.error_codes]))
        response = PublicResponse(
    ...
    if outcome.status == "COMPLETE":
        ...
        for goal in updated:
            if goal.status == "PENDING":
                ...
                settled.append(goal)
        return settled, assumed
    return updated, assumed
