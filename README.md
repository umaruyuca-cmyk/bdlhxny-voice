# Touchstone 受控 Agent 实验平台

本仓库用于在固定问题、固定上下文和冻结工具数据下运行可复查的 Agent 实验。正式实验统一使用原生 Tool Calling 执行底座，通过版本化模板一次只改变一个主要变量，例如治理策略、工具提供方式、上下文组织、温度或最大步数。
...
```text
engine/  私有运行 API、统一 Agent 循环、实验模板、上下文构建与评测
