...
public class JwtTokenProvider {
    ...
    public JwtTokenProvider(
            @Value("${stockwise.jwt.secret:}") String secret,
            @Value("${stockwise.jwt.expiration-days:7}") int expirationDays) {
        byte[] keyBytes = (secret.isBlank()
                ? "StockWise-Default-JWT-Key-2026-Must-Override-In-Production!!!"
