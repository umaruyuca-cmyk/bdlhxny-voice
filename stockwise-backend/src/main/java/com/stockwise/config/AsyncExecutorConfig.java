...
public class AsyncExecutorConfig {
    ...
    public ExecutorService agentFlowExecutor(
            @Value("${stockwise.execution.agent.core-threads:4}") int coreThreads,
            @Value("${stockwise.execution.agent.max-threads:16}") int maxThreads,
            @Value("${stockwise.execution.agent.queue-capacity:100}") int queueCapacity) {
        return boundedExecutor(
                coreThreads, maxThreads, queueCapacity, "stockwise-agent-");
    }
    ...
    public ExecutorService reactToolExecutor(
            @Value("${stockwise.execution.react-tool.core-threads:4}") int coreThreads,
            @Value("${stockwise.execution.react-tool.max-threads:8}") int maxThreads,
            @Value("${stockwise.execution.react-tool.queue-capacity:64}") int queueCapacity) {
        return boundedExecutor(
                coreThreads, maxThreads, queueCapacity, "stockwise-react-tool-");
    }
    ...
    public ExecutorService routingClassifierExecutor(
            @Value("${stockwise.routing.semantic.max-concurrent-calls:8}") int maxConcurrentCalls) {
        int concurrency = Math.max(1, maxConcurrentCalls);
        return boundedExecutor(
                concurrency, concurrency, concurrency, "stockwise-routing-");
    }
