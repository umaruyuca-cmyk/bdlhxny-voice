...
public class HealthController {
    ...
    @Value("${stockwise.embedding.provider:dashscope}")
    private String embeddingProvider;

    @Value("${stockwise.embedding.model:text-embedding-v4}")
    ...
    public Map<String, Object> ping() {
        ...
        // 1. 依次探测四个依赖
        result.put("service", "stockwise-backend");
    ...
    private Map<String, Object> checkEmbedding() {
        try {
            // 1. 使用固定短文本执行真实向量请求。
            float[] vector = embeddingService.embed("StockWise健康检查");
            return Map.of(
                    "status", "up",
