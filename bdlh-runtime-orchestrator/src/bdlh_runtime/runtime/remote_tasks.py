...
class RemoteTaskStore:
    ...
    def create(self, task: FinancialTask) -> FinancialTask:
        return _task(self._client.call("POST", "/internal/v1/tasks", task.authenticated_user_id,
                                      payload=_save_payload(task)))

    def update(self, task: FinancialTask, *, expected_version: int) -> FinancialTask:
        return _task(self._client.call(
            "PUT", f"/internal/v1/tasks/{task.task_id}", task.authenticated_user_id,
    ...
    def get(self, task_id: str, authenticated_user_id: str) -> FinancialTask | None:
        data = self._client.call("GET", f"/internal/v1/tasks/{task_id}", authenticated_user_id,
    ...
    def list_for_user(self, authenticated_user_id: str, *, limit: int = 100) -> list[FinancialTask]:
        data = self._client.call("GET", "/internal/v1/tasks", authenticated_user_id,
    ...
    def claim_due(self, *, now: datetime, limit: int) -> list[tuple[FinancialTask, str]]:
        ...
        data = self._client.call_internal("POST", "/internal/v1/task-scheduler/claim-due",
                                          query={"limit": max(1, limit)})
        if not isinstance(data, list):
    ...
    def complete_task_and_enqueue_notification(
        ...
        *,
        ...
        self._client.call(
            "POST", f"/internal/v1/tasks/{task.task_id}/complete-notification",
...
class RemoteNotificationOutbox:
    ...
    def list_for_user(self, authenticated_user_id: str, *, limit: int = 100) -> list[NotificationOutboxMessage]:
        data = self._client.call(
            "GET", "/internal/v1/notifications", authenticated_user_id,
            query={"limit": max(1, limit)},
        )
        if not isinstance(data, list):
...
def create_remote_task_stores(*, base_url: str | None, internal_token: str | None) -> tuple[RemoteTaskStore, RemoteNotificationOutbox]:
    if not base_url:
        raise RuntimeError("BDLH_RUNTIME_DATA_MODE=java requires JAVA_API_BASE_URL")
