...
VALUES
  ...
  -- 固定复合 Deep Research（ADR-016）：进程内执行器，私有原子搜索→百炼；默认 Flag 关闭时不进 allowed
  ('research.deep_search', '对研究任务做多轮拆题、检索、压缩并返回结构化 ResearchBundle', 'research', 'local', TRUE, FALSE,
   '{question,objective}', '{}', 'ResearchBundleObservation', 90, 5, TRUE),
