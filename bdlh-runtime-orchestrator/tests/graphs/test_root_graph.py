...
def test_dynamic_snapshot_workflow_completes_with_mock_data():
    graph = build_root_graph(registry_snapshot=seeded_snapshot())
...
def test_direct_response_knowledge_question_no_tool():
    ...
    # 重写：知识问答 needs_external=false → 无工具回答（不再有 intent_route）
    assert result["final_response"] is not None
    assert result["final_response"]["mode"] == "no_tool_response"
...
def test_comprehensive_no_symbol_not_interrupted():
    ...
    # 重写语义：无标的的研究请求是真缺口 → ASK_USER（interrupt），不静默编标的
...
def test_cross_turn_symbol_inheritance():
    ...
    assert result2["understand"]["entities"]["instruments"] == ["600519"]
